  ...
  <div class="app-container">
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-divider content-position="center">管理商品信息</el-divider>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleAddGood">添加</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" icon="el-icon-delete" size="mini" @click="handleDeleteGood">删除</el-button>
          </el-col>
        </el-row>
  ...
  methods: {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.form.goodList = this.goodList
    ...
    handleDeleteGood() {
      if (this.checkedGood.length == 0) {
        this.$modal.msgError("请先选择要删除的管理商品数据")
      } else {
        const goodList = this.goodList
        const checkedGood = this.checkedGood
        this.goodList = goodList.filter(function(item) {
          return checkedGood.indexOf(item.index) == -1
        })
      }
    },
    /** 复选框选中数据 */
    handleGoodSelectionChange(selection) {
      this.checkedGood = selection.map(item => item.index)
    },
    /** 导出按钮操作 */
    handleExport() {
      this.download('good/type/export', {
        ...this.queryParams
